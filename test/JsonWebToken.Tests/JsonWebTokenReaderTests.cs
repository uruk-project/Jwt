...
namespace JsonWebToken.Tests
{
    public class JwtDocumentTests : IClassFixture<KeyFixture>, IClassFixture<TokenFixture>
    {
        ...
        public void ReadJwt_ValidMultipleSequence(string token, bool signed)
        {
            ...
            var result = JwtDocument.TryParse(sequence, builder, out var document);
            Assert.True(result);
            Assert.Equal(TokenValidationStatus.Success, document.Error.Status);
        }
        ...
        public void ReadJwt_Valid(string token, bool signed)
        {
            ...
            var result = JwtDocument.TryParse(jwt, builder, out var document);
            Assert.True(result);
            Assert.Equal(TokenValidationStatus.Success, document.Error.Status);
        }
        ...
        public void ReadJwt_Invalid(string jwt, TokenValidationStatus expectedStatus)
        {
            ...
            var result = JwtDocument.TryParse(jwt, policy, out var document);
            Assert.False(result);
        ...
        public void ReadJwt_CriticalHeader_Invalid(string jwt, TokenValidationStatus expected, bool headerCacheEnabled)
        {
            ...
            var result = JwtDocument.TryParse(jwt, policy.Build(), out var document);
            Assert.False(result);
