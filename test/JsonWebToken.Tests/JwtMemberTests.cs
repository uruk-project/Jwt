...
namespace JsonWebToken.Tests
{
    public class JwtMemberTests
    {
        ...
        {
            Assert.Throws<ArgumentNullException>(() => new JwtMember("x", (object)null));
        }

        public static IEnumerable<object[]> GetPropertiesToWrite()
        {
            yield return new object[] { new JwtMember("object", new object()), "{\"object\":{}}" };
            yield return new object[] { new JwtMember("array", new List<object>()), "{\"array\":[]}" };
            yield return new object[] { new JwtMember("int64", 1L), "{\"int64\":1}" };
            yield return new object[] { new JwtMember("int32", 1), "{\"int32\":1}" };
            yield return new object[] { new JwtMember("int16", (short)1), "{\"int16\":1}" };
            ...
            yield return new object[] { new JwtMember("double", 1.1d), "{\"double\":" + Encoding.UTF8.GetString(destination.AsSpan().Slice(0, bytesWritten)) + "}" };
#endif
            yield return new object[] { new JwtMember("string", "hello"), "{\"string\":\"hello\"}" };
            //yield return new object[] { new JwtMember("utf8String", new[] { (byte)'h', (byte)'e', (byte)'l', (byte)'l', (byte)'o' }), "{\"utf8String\":\"hello\"}" };
            yield return new object[] { new JwtMember("boolean", true), "{\"boolean\":true}" };
        }
    }
}
