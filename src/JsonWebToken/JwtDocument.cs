...
namespace JsonWebToken
{
    ...
    public sealed class JwtDocument : IDisposable
    {
        ...
        public JwtDocument(JwtHeader header, ReadOnlyMemory<byte> rawValue, byte[] rented)
        {
            Header = header;
            _rawValue = rawValue;
            _rented = rented;
        ...
        public JwtDocument(TokenValidationError error)
        {
            Error = error;
        }

        public JwtDocument(JwtHeader header, JwtPayload payload)
        {
            Header = header;
            _payload = payload;
        }

        public JwtDocument(JwtHeader header, JwtPayload payload, TokenValidationError error)
        {
            Header = header;
            _payload = payload;
            Error = error;
            ...
            try
            {
                ...
                if (validHeader)
                {
                    return segmentCount switch
                    {
                        ...
                        Constants.JweSegmentCount => TryReadJwe(utf8Token, policy, rawHeader, ref segmentsRef, header, out document),
