...
namespace JsonWebToken.Cryptography
{
    internal class Pkcs8
    {
        ...
        {
            const string PublicKeyPrefix = "-----BEGIN PUBLIC KEY-----";
            const string PublicKeySuffix = "-----END PUBLIC KEY-----";
            ...
            //var reader = reader.ReadSequence();
            reader = reader.ReadSequence();
            var readerOid = reader.ReadSequence();
            var oid = readerOid.ReadOid();
            readerOid.ReadNull();
            Debug.Assert(oid.Length == 7);
            Debug.Assert(oid[0] == 1);
            Debug.Assert(oid[1] == 2);
            Debug.Assert(oid[2] == 840);
            Debug.Assert(oid[3] == 113549);
            Debug.Assert(oid[4] == 1);
            Debug.Assert(oid[5] == 1);
            Debug.Assert(oid[6] == 1);
