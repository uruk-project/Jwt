...
namespace JsonWebToken.Internal
{
    internal sealed class Aes256NiCbcEncryptor : AesEncryptor
    {
        ...
        public override void Encrypt(ReadOnlySpan<byte> key, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, Span<byte> ciphertext)
        {
            ...
            var keys = new Aes256EncryptionKeys(key);
            ...
            if (!plaintext.IsEmpty)
            {
                ...
                while (Unsafe.IsAddressLessThan(ref input, ref inputEnd))
                {
                    ...
                    state = Aes.Encrypt(state, keys.Key12);
                    state = Aes.Encrypt(state, keys.Key13);
                    state = Aes.EncryptLast(state, keys.Key14);
    ...
    internal sealed class Aes256NiBlockEncryptor : AesBlockEncryptor
    {
        private readonly Aes256EncryptionKeys _keys;

        public Aes256NiBlockEncryptor(ReadOnlySpan<byte> key)
        {
