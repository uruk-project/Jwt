...
namespace JsonWebToken.Internal
{
    internal sealed class Aes128NiCbcEncryptor : AesEncryptor
    {
        ...
        public override void Encrypt(ReadOnlySpan<byte> key, ReadOnlySpan<byte> plaintext, ReadOnlySpan<byte> nonce, Span<byte> ciphertext)
        {
            ...
            var keys = new Aes128EncryptionKeys(key);
            ...
            if (!plaintext.IsEmpty)
            {
                ...
                while (Unsafe.IsAddressLessThan(ref input, ref inputEnd))
                {
                    ...
                    state = Aes.EncryptLast(state, keys.Key10);
    ...
    internal sealed class Aes128NiBlockEncryptor : AesBlockEncryptor
    {
        private readonly Aes128EncryptionKeys _keys;

        public Aes128NiBlockEncryptor(ReadOnlySpan<byte> key)
        {
